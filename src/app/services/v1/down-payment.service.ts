...
import { DownPaymentState, DownPaymentType, JournalSourceType, JournalState, MASTER_ROLES, PeriodState, } from '../../../model/utils/enum';
import { DownPaymentResponse, DownPaymentsWithPaginationResponse, ShowDownPaymentResponse } from '../../domain/down-payment/down-payment-response.dto';
/** Collections */
...
export class DownPaymentService {
    constructor(
        @InjectRepository(DownPayment) private readonly downPayEntity: Repository<DownPayment>,
        @InjectRepository(DownPaymentHistory) private readonly historyDownPayEntity: Repository<DownPaymentHistory>,
    ) { }
    ...
    public async getAll(query?: QueryDownPaymentDTO,): Promise<DownPaymentsWithPaginationResponse> {
        try {
    ...
    public async createDownPayment(payload: CreateDownPaymentDTO): Promise<DownPaymentResponse> {
        try {
            const create = await getManager().transaction(async (manager) => {

                let date = new Date(payload.transactionDate),
                    month = date.getMonth() + 1,
                    year = date.getFullYear()
                    
                const periodEntity = manager.getRepository<Period>(Period);
                const period = await periodEntity.findOne({ month: month, year: year, isDeleted: false});
                
                console.log(date, month, year, period);
                if (!period || (period && period.state === PeriodState.CLOSE)) {
                    throw new BadRequestException(`Failed create due period already closed!`);
                }
                ...
                await this.createHistory(downPayment, { state: DownPaymentState.DRAFT, downPaymentId: result.id });
    ...
    public async approveDownPayment(downPaymentId: string,): Promise<any> {
        try {
            const approve = await getManager().transaction(async (manager) => {
                ...
                const downPayment = await manager.findOne(DownPayment, { where: { id: downPaymentId, isDeleted: false }, relations: ['branch', 'employee', 'department', 'histories'], });
                ...
                await this.createHistory(downPayment, { state, downPaymentId: result.id });
    ...
    public async rejectDownPayment(id: string, payload: RejectDownPaymentDTO): Promise<any> {
        try {
            const reject = await getManager().transaction(async (manager) => {

                const downPayment = await manager.findOne(DownPayment, { where: { id: id, isDeleted: false }, relations: ['branch', 'employee', 'department', 'histories'], });
                if (!downPayment) throw new NotFoundException(`Down payment ID ${id} not found!`);
                ...
                ) { throw new BadRequestException(`Only PIC/SS/SPV HO can reject down payment!`); }
                ...
                await this.createHistory(downPayment, { state, downPaymentId: result.id, rejectedNote: payload.rejectedNote });
    ...
    private async createHistory(downPayment: DownPayment, data?: { state: DownPaymentState; downPaymentId: string, rejectedNote?: string; }): Promise<DownPaymentHistory[]> {
        try {
            ...
            const mergeHistory = [].concat(downPayment.histories, [history,]) as DownPaymentHistory[];
    ...
    private async removeJournal(manager: EntityManager, data: DownPayment,): Promise<any> {
        try {
            const jurnalEntity = manager.getRepository<Journal>(Journal);

            const journal = await jurnalEntity.find({ sourceType: JournalSourceType.DP, reference: data.number });
            const nonDraftJournal = journal.length > 0 ? journal.filter((v) => v.state !== JournalState.DRAFT) : []

            if (nonDraftJournal.length > 0) {
                throw new BadRequestException(`Can't remove journal because there are any journal entries with state not draft!`,);
            }

            return await jurnalEntity.delete({ sourceType: JournalSourceType.DP, reference: data.number });
    ...
    private async createJournal(manager: EntityManager, downPayId: string, userRole: MASTER_ROLES,): Promise<any> {
        try {

            const jurnalEntity = manager.getRepository<Journal>(Journal);
            const periodEntity = manager.getRepository<Period>(Period);

            const downPayment = await manager.findOne(DownPayment, { where: { id: downPayId, isDeleted: false }, relations: ['branch'], });
            const period = await periodEntity.findOne({ id: downPayment.periodId, isDeleted: false });

            if (!period || (period && period.state === PeriodState.CLOSE)) {
                throw new BadRequestException(`Failed create journal due period already closed!`);
            }
    ...
    private async createJournalItemDebit(manager: EntityManager, downPayment: DownPayment, jurnalId: string): Promise<JournalItem> {
        try {
            const jurnalItemEntity = manager.getRepository<JournalItem>(JournalItem);
            const globalSettingEntity = manager.getRepository<GlobalSetting>(GlobalSetting);
    ...
    private async createJournalItemCredit(manager: EntityManager, downPayment: DownPayment, jurnalId: string): Promise<JournalItem> {
        try {
            const jurnalItemEntity = manager.getRepository<JournalItem>(JournalItem);
