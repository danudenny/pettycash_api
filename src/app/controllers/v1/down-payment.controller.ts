import { Body, Controller, Get, HttpException, HttpStatus, NotFoundException, Param, ParseUUIDPipe, Patch, Post, Query } from '@nestjs/common';
import { ApiBadRequestResponse, ApiBody, ApiCreatedResponse, ApiHeader, ApiInternalServerErrorResponse, ApiNotFoundResponse, ApiOkResponse, ApiOperation, ApiTags } from '@nestjs/swagger';
/** Interfaces */
...
import { DownPaymentResponse, ShowDownPaymentResponse, DownPaymentsWithPaginationResponse } from '../../domain/down-payment/down-payment-response.dto';
/** Services */
...
export class DownPaymentController {
    ...
    @ApiCreatedResponse({type: DownPaymentResponse, description: 'Expense Successfully Created'})
    ...
    @ApiBadRequestResponse({ description: 'Bad Request' })
    async getAll( @Query() query: QueryDownPaymentDTO,): Promise<DownPaymentsWithPaginationResponse>{
        try {
    ...
    async show(@Param('id', new ParseUUIDPipe()) id: string) {
        try {
            const dwnPy= await this.downPaymentService.findOne({ id, isDeleted: false })
            if (!dwnPy) throw new NotFoundException({
                status: "fail",
    ...
    @ApiBody({ type: Object })
    async approveDownPaymant(@Param('id', new ParseUUIDPipe()) id: string, @Body() actApprove: object ) {
        try {
    ...
    async rejectDownPaymant(@Param('id', new ParseUUIDPipe()) id: string, @Body() actReject: RejectDownPaymentDTO ) {
        try {
